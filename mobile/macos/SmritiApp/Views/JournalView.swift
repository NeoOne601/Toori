        ...
        VStack(spacing: 0) {
            ...
            ZStack {
                if let entry = engine.cachedJournal(for: selectedDate) {
                    ScrollView {
                        // TODO: Add share button integrating MemoryCardGenerator later
                        Text(entry)
                            ...
                            .onAppear {
                                ...
                                if !UserDefaults.standard.bool(forKey: key) {
                                    showAnimation = false
                                    // Normally we would split and stagger words here. For brevity, animate entire text.
        ...
        Task {
            ...
            if let cardURL = generator.generateMemoryCard(imagePath: nil, date: selectedDate, summary: entry) {
                await MainActor.run {
                    ...
                    #if os(macOS)
                    NSSharingServicePicker(items: [cardURL]).show(relativeTo: .zero, of: NSApp.keyWindow?.contentView ?? NSView(), preferredEdge: .minY)
                    #endif
                }
            } else {
                await MainActor.run {
